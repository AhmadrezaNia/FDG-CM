...
class MessageConsumer:
    def __init__(self):
        ...
        self.train_class =  [1,4,7]
        self.data = np.load('All.npy')
        
        self.Clinet_name = 1
        self.Client_num = 1 # The share section number 
        self.num_of_clients = 3  # number of clients have same data set ( class_wise)
        self.routing_key = "Surface_6"
    ...
    def load_data(self):
        ...
        x = self.data[:, 1:]  # Input features
        y = self.data[:, 0]  # Labels

        # Map the class labels
        mapped_labels = np.array([self.label_mapping[label] for label in y])

        # Perform feature scaling
        # Find the indices of labels that belong to the test classes
        indices_set_train = np.where(np.isin(y, self.train_class))[0]
        indices_set_test = np.where(np.isin(y, self.test_class))[0]
        # Create the training set by excluding the indices found in the test set
        y_test= keras.utils.to_categorical(mapped_labels[indices_set_test], num_classes=self.layers[-1])
        y_train1 = keras.utils.to_categorical(mapped_labels[indices_set_train], num_classes=self.layers[-1])

        # Separate the training and test sets
        x_test1 = x[indices_set_test]
        x_train1 = x[indices_set_train]
        ...
        y_train = y_train1[self.Client_num * data_per_client : (self.Client_num + 1) * data_per_client] # Labels 
